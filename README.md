...
### Структура хранилища:
	* данные по событиям будут считываться из директории `/user/master/data/geo/events`
	* тестироваться код будет в директории `/user/yussdan/data/tmp`
	* директория для аналитиков с данными `/user/yussdan/analytics`
	* даг будет обновлять данные раз в неделю
